            ...
            if not ek.ek(os.path.isfile, curLoc) or not os.path.normpath(curLoc).startswith(os.path.normpath(self.location)):
                ...
                with curEp.lock:
                    ...
                    if curEp.location and curEp.status in Quality.DOWNLOADED:
                        logger.log(str(self.tvdbid) + u": Location for " + str(season) + "x" + str(episode) + " doesn't exist, removing it and changing our status to ARCHIVED", logger.DEBUG)
        ...
        if not ek.ek(os.path.isfile, self.location):
            ...
            # if there's no airdate then set it to skipped (and respect ignored)
                ...
                if self.status == IGNORED:
                    ...
                    logger.log(u"Episode has no air date, assume it never aired", logger.DEBUG)
                    self.status = UNAIRED
